    ...
    `}tempToColor(e){if(null==e)return"#64b5f6";const t=(Math.max(50,Math.min(140,e))-50)/90;if(t<.33){return`rgb(${Math.round(30+3*t*70)},${Math.round(130+3*t*70)},${Math.round(245-3*t*50)})`}if(t<.66){const e=(t-.33)/.33;return`rgb(${Math.round(100+155*e)},${Math.round(200-50*e)},${Math.round(195-145*e)})`}{const e=(t-.66)/.34;return`rgb(${Math.round(255)},${Math.round(150-80*e)},${Math.round(50-30*e)})`}}renderFloorSystem(){const e=this.floorSystem;if(!e)return null;const t=e.return_temp?.value,r=e.supply_temp?.value,s=this.tempToColor(t),o=this.tempToColor(r),i=e.return_temp?.unit??"°",n=e.supply_temp?.unit??"°";return D`
      <div class="floor-system tankless-visual">
        <svg class="tankless-svg" viewBox="0 0 400 160" preserveAspectRatio="xMidYMid meet">
          ...
          <rect x="0" y="105" width="130" height="14" rx="7" fill="rgba(100,181,246,0.15)" stroke="${s}" stroke-width="1.5"/>
          <rect x="5" y="107" width="120" height="10" rx="5" fill="url(#inletFlow)"/>

          <!-- Outlet pipe (hot, top) -->
          <rect x="270" y="45" width="130" height="14" rx="7" fill="rgba(255,183,77,0.15)" stroke="${o}" stroke-width="1.5"/>
          ...
          <path d="M 155 140 L 155 112 Q 130 112 130 112" fill="none" stroke="${s}" stroke-width="2" opacity="0.6"/>
          <!-- Outlet connection (top-right of heater to outlet pipe) -->
          <path d="M 245 40 L 245 52 Q 270 52 270 52" fill="none" stroke="${o}" stroke-width="2" opacity="0.6"/>

          <!-- Inlet temp label -->
          <text x="60" y="100" font-size="11" text-anchor="middle" fill="${s}" font-family="sans-serif" font-weight="700">${null!=t?`${t}${i}`:"—"}</text>
          ...
          <!-- Outlet temp label -->
          <text x="340" y="40" font-size="11" text-anchor="middle" fill="${o}" font-family="sans-serif" font-weight="700">${null!=r?`${r}${n}`:"—"}</text>
          ...
          ${null!=e.delta_t?D`
                ...
                <text x="200" y="145" font-size="9" text-anchor="middle" fill="rgba(255,255,255,0.7)" font-family="sans-serif" font-weight="600">ΔT ${e.delta_t}°</text>
              `:""}

          <!-- Flow rate (if available) -->
          ${e.flow_rate?D`
                <text x="200" y="15" font-size="8" text-anchor="middle" fill="rgba(255,255,255,0.5)" font-family="sans-serif">Flow: ${e.flow_rate.value} ${e.flow_rate.unit}</text>
              `:""}

          <!-- Pump status (if available) -->
          ${null!=e.pump_entity?D`
                <circle cx="385" cy="52" r="5" fill="${e.pump_active?"#4caf50":"#616161"}"/>
